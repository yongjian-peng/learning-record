...
- RDB 全量日志，有两种方法，一种阻塞，一种非阻塞。写时复制，对修改的数据，生成一个副本，进行修改，做增量快照。AOF + RDB 组合保存日志，问题恢复的很好，多写少读 8:2 的情况，会有内存 cpu 的影响。不过一点需要注意，同时开启 RDB + AOF 不要绑定内核cpu .
#### 06 | 数据同步：主从库如何实现数据一致？
...
- List, 可以，执行失败，宕机时候，可以保存消息。可以阻塞消息使用 BRPOP 命令。
