...
- 2 事务隔离级别，读未提交，读提交，可重复读，可串行化。
  - 读提交的问题，列表展示员工数量，读取情况下，并发，有一个事务修改了，男性为女性。就导致了男性多了一条数据。
  - 可重复读的问题，会产生幻读的问题，并发事务中，A事务查询 -> B事务插入 -> A事务修改（包含A事务新插入的数据） -> 导致B的数据也会被修改掉
  - 幻读引出的问题是，放到从库执行的时候，会导致数据不一致的情况。
...
- RR隔离级别下，为保证binlog记录顺序，非索引更新会锁住全表记录，且事务结束前会对,不符合条件记录,有逐步释放的过程。
=======
- RR事务隔离级别，for update 并发情况，会锁表（待议，应该是锁间隙），提交后，才会释放锁。
- RR隔离级别下，为保证binlog记录顺序，非索引更新会锁住全表记录，且事务结束前不会对不符合条件记录有逐步释放的过程。
>>>>>>> 176ff4588ead24b424e97fdfe148013d44531ed6
#### 20 | 幻读是什么，幻读有什么问题？******* 重点*******
...
#### 24 | MySQL是怎么保证主备一致的？
- Mysql 设计成binlog 的保存格式， statement row 格式，当备库追赶主库的情况，设置成非双一。保证快速执行备库的数据。
#### 25 | MySQL是怎么保证高可用的？
- 同步延迟时间。主备延迟：备库消费 relay log 日志的速度，比主库生产 binlog 的速度差。
- 主备切换，可用性切换，有数据不一致的情况发送。
- 备库压力大，大事务。（删除多数据），DDL,备库并行复制的能力。
- 循环复制的问题，手动修改 serverid 判断不是自己的，会导致循环写入，还有，其他库生成的 binlog 去为双主的节点上执行也会循环复制。
- 一主一备的双M结构，主备切换只需要把客户流量切到备库；一主多从架构里，主备切换除了把客户端流量切到备库外，还要把从库接到新的主库上。
#### 26 | 备库为什么会延迟好几个小时？
- 备库并行复制的能力。多个线程。事务分发的情况下，
- 按照表分发，按照行分发，（热点表的情况）
- Mysql 5.7的优化并行，当处于 redo log prepare 阶段，可以并行，剩下的 处于redo log prepare 和 commit 阶段的，可以并行。
#### 27 | 主库出问题了，从库怎么办？
- 基于位点的，切换主库成功后，从库执行位点的情况，报错情况跳过。
- 基于GTID的方式切换，GTID包含service_id 和 事务ID  GTID 主键冲突的情况下，set gtid_next = "xxx" 即跳过当前的事务。可以主备加DDL的情况，跳过同时加入DDL, 然后再切换主备。
#### 28 | 读写分离有哪些坑？
- 过期读：主库刚写入，就查询从库的数据。
- 解决方法，延迟查询。基于位点GTID,group 查询差集的方式去处理。根据返回的gtid 去查询是否从库执行成功，从库执行成功，查从库。执行失败，查询主库。
#### 29 | 如何判断一个数据库是不是出问题了？
- select 1 返回不能判定是否正常使用，遇到大事务的情况下。需要修改表记录，才能判定是否正常。在每个节点，一定频率执行修改表，修改当前时间。即可判断是否正常使用。
- 根据磁盘利用率，内部统计是否可用。查询内部库中的io 时间，根据这个返回，不过降低10% 性能。
#### 30 | 答疑文章（二）：用动态的观点看加锁
#### 31 | 误删数据后除了跑路，还能怎么办？
- 判断开启binlog, 多个库保证数据的多副本，延迟备份一天的数据，发生问题后，开启 gtid ，把问题的数据改删掉。再放入执行。
#### 32 | 为什么还有kill不掉的语句？
- innodb_thread_concurrency 并行查询的数量。给占用了，就导致Kill不掉。
#### 33 | 我查这么多数据，会不会把数据库内存打爆？
#### 34 | 到底可不可以使用join？
- 走了查询条件的 索引的情况下还是能够使用 join 查询的。
- join 中的小表做驱动表，参与的条件 where 查询和查询的表中的字段多少。有关系。
- join buffer 中的配置设置的比较小的情况下。会导致缓存命中率极低。读取的数据会移动到缓存区 young 区。
#### 35 | join语句怎么优化？
- 根据磁盘的顺序读的特性。join 查询 是范围查询的情况下的使用的优化。
- 查询到的符合条件的数据比较小的情况下，可以使用创建临时表。临时表中加入索引，然后在join 查询语句即可。
#### 36 | 为什么临时表可以重名？
#### 37 | 什么时候会使用内部临时表？
