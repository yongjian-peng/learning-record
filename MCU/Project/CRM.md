...
27. 当python_api 服务不可用，或者 mqtt 不可用，或者 influxdb 不可用的时候，则做出止损的动作，应该怎么设计比较合理。如果python_api 不可用的话，则当前客户端在执行的任务。怎么处理。 api 已不可用的话。api 是关键，很多功能都需要提供服务支持。怎么解决比较好。01_qt_qml_client 02_agent_core_cpp 部署客户端 03_backend_python_api 04_web_admin_vue 部署在服务器
...
45. 原来的首页保持不变，继续使用新的页面。/dashboard 综合监控页面，使用 将 3D 模型，更换上E:\WorkSpace\CRM001\dist\recovered-src 项目中的首页样式。先替换页面。
...
56. 绿色版本当刚拷贝到客户机时候，连接本地数据失败。连接多次就可以了。当断线后，操作了开启压力测试软件，关闭测试软件，然后连上服务器。服务器控制软件。显示command result timeout，然后再设置中重新点击确定后，从服务端发送操作压力测试软件才可以。 
57. 绿色版本，客户端连接上服务器后，同步数据，显示 http://127.0.0.1:9801/api/v1/telemetry/lastest  {"error":"route not found","success":false} OK  
58. 本地数据，有CPU 内存使用率，采集数据，需要有 CPU 温度，硬盘 硬盘温度。网络使用率。OK 
59. 客户端老化控制，当连接上网的时候，则将老化数据，发送到后端。区分断网和联网时候。看下记录的时间。不能是0.将所有关于切换的的逻辑，封装成一个单独的文件。
专门处理刚开机是否有连接成功服务器，有连接服务器成功执行的逻辑，将数据同步到服务器，并停止本地数据的记录。如果连接失败服务器的逻辑。将本地数据记录到 SQLite 中，并按照一定频率检查是否成功连接服务器。封装成调用函数即可。不改变之前的逻辑。就是切换网络的顶层逻辑。不牵扯到具体的细节。细节还是之前的逻辑。抽成单独的方法。这个文件中调用即可。是逻辑清晰。
60. 本地数据 历史采集的数据，使用时间倒序排序。
61. 绿色版本，日志更新日志级别，waring dubug info， 可以设置日志等级，减少日志大小。OK 
...
69. 验证同步的数据，保存到数据库中，数据是否错误或者混乱，将保存的 Mysql 的数据，可以显示已图形化的方式显示。列表数据保留。新增一个图形化页面。和前端页面保持一致。使用简单的数据结构。不要使用复杂的数据结构保存数据。
70. 新增 AIDA64 软件的开启，之前的BrunInTest 的逻辑不改动。
...
71. 当拷机CPU温度为100度时候，点击设置获取设备ID,卡死在当前页面中。做一个优化。绿色版本当机器运行其他拷机软件时候，设置点击获取生成的设备ID, 页面卡死了。
72. 采集到的 CPU 温度为 100度，这个温度是否有限制，不是真实的温度。如果有限制的话，去掉限制，显示采集到真实的温度。
73. qt_qml 颜色可以切换主体。防止屏幕一直保持在一个颜色下。
74. 后端批量操作时候，修复启动buruintest 软件bug, 批量操作开始老化bug. vue 请求的是 /api/tasks/burnintest 路径
75. 说明 GpuCollector 或 LibreHardwareMonitor 底层调用长时间没有退出。
