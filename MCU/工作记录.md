...
```
RK3568 鼠标插上 OTG 接口 鼠标没有生效。
	原因是 OTG 需要关闭才行。
DIDO 24V 电源驱动能力
	DO1-DO3 触发时输出的是24v 电压，DO4-DO5，输出上升沿时与GND测出的电压是22v，接上继电器后再测电压是9.3v。是硬件设计的缺陷。
	
	
通过网盘分享的文件：Ubuntu_20.04.6_LTS_HW_ext4_20260327095544.img
链接: https://pan.baidu.com/s/1EeYyIZIyuvtc-feLwG8dkQ?pwd=3gqd 提取码: 3gqd 
--来自百度网盘超级会员v6的分享

摄像头使用 QY-3588-20260408 镜像
Z:\主板测试记录\主板测试记录\HW汇湾\安卓主板\QY-3588\固件\安卓13\标准系统+电阻触摸屏
...
```
重启设备验证   QY3588_Ubuntu20_YK_GNOME-ubuntubase-overlayroot-260717.img
Z:\整机研发测试记录\旧文件\非标\合肥\周磊\安徽研信  3588  215平板\工艺文件

Ubuntu 22.04
Z:\主板测试记录\主板测试记录\HW汇湾\安卓主板\QY-3588\固件\LINUX\ubuntu 22.04  

更换logo 杭州博创智信技术有限公司 QY-3588-S-XFCE-01-260720.img
Z:\整机研发测试记录\旧文件\非标\郑州\岑聪聪\杭州博创智信技术有限公司\QY-B4000-RK3588-OEM 16G+128G\工艺文件
```
...
```
网络问题。客户使用组网，日志分析的结果。 安装了看门狗了。代验证


信步 reboot 后黑屏问题。追踪 待从现场寄回设备。 已经收到设备
C盘文件 写满的问题。 重新安装系统。

等待8月6号询问进展 API 开发。 给到不完整的，更新了镜像了。 这周五给到新的需求。OK 
RK3568 镜像定制  上海华智极星人工智能科技有限公司 
修改 U-Boot CMA 内存增加。OK 
汇弯arm 3588 22.04加logo系统盯下汇弯下周给改出来 ，验证好了跟乔工说下 OK 
同屏异显的 下周三 7月22号 答复进度。 OK  客户自己开发 api 
```
...
```
 ...
 xinput set-prop "My USB Touchscreen" "Coordinate Transformation Matrix" 0 1 0 -1 0 1 0 0 1
 xinput set-prop "ILITEK ILITEK-TP Mouse" "Coordinate Transformation Matrix" 0 1 0 -1 0 1 0 0 1
 
 
 目前异常重启的，启阳+我们+科亿公司+科亿现场
```

![7ce2957e9b28f550e4c16653cb72b547](E:\Wechat\xwechat_files\wxid_92xnmuiuyff621_ae70\temp\RWTemp\2026-07\7ce2957e9b28f550e4c16653cb72b547.png)
...
```
1.找到查找 WiFi 驱动名称
lsmod | grep -E "wlan|brcm|rtl|88x2|bcmdhd|cfg80211"
2.置开机自动卸载驱动
sudo sh -c "echo 'rmmod brcmfmac' >> /etc/rc.local"
sudo sh -c "echo 'rmmod brcmutil' >> /etc/rc.local"
...
```
"/d/Qt/Tools/CMake_64/bin/cmake.exe" --build . --target install
cd "/d/WorkSpace/QT-Github/shotcut-win64-sdk-26.4.30/Shotcut/src/shotcut/build/Desktop_Qt_6_10_0_MinGW_64_bit-Debug"
...
```
RK3588 安卓13能支持 Widevine L1 DRM吗

RK3568 @吴思含 吴工这款主板 有几个问题帮忙确认下
操作系统支持：Android 11+ 或已预装 LineageOS / postmarketOS / Armbian 系统的机型。必须具备安装 Linux 系统并支持 SSH 连接的能力。
- Bootloader：已解锁，或具备已记录的解锁步骤。
- Recovery：可通过音量键 + 电源键或 USB fastboot 进入。
- 严格禁止：禁止永久锁定 Bootloader、强制安装 Google 认证，以及在启动分区中安装无法移除的间谍软件。
Wi-Fi 规格：必须支持 AP 模式/SoftAP/hostapd

- 操作系统支持：只支持Android 11+ ， LineageOS / postmarketOS / Armbian 系统不支持，Linux支持的是Ubuntu，Ubuntu支持SSH连接
- Bootloader：是解锁的，没有什么限制
- Recovery：可进入
- 严格禁止：Bootloader没有限制、Google认证是可选的、无其他间谍软件
- WiFi：AP模式有bug，不建议使用
...
1.硬盘  128GB eMMC 5.1，支持
2.喇叭  8W 4Ω ，需要调试喇叭的输出功率
3.工作温度 -25℃ ～ +70℃，主板的主要器件要替换宽温物料
4.湿度 ≤95%（无凝露），需要刷三防漆
5.海拔≤5100m ，这个主要是低压对风扇有影响，如果你们整机不加风扇，问题应该不大

1. Configuration of UEFIUEFI 固件配置
2. Customer Secure Boot客户自定义安全启动
3. Our PK/KEK/db key enrolment录入我方PK、KEK、DB安全密钥
4. We would like to have TPM ownership/control.我方需取得TPM芯片所有权与完全管控权限
5. BSP source access
开放BSP板级支持包源码权限
1. 3568不支持UEFI
2&3&4. 无TPM芯片
...
20260628 第二版调试记录
  以调试
	1. 系统版本android11 debian
	2. 添加uart3
    3. 加强edp gpio 推动能力到5级
	4. 测试f_usb1 host 功能 命令如下
	echo host > /sys/devices/platform/fe8a0000.usb2-phy/otg_mode
	echo otg > /sys/devices/platform/fe8a0000.usb2-phy/otg_mode
	5. 添加lvds reset 功能，不执行脚本可以使用
	6. 修改tf 最大频率为兼容更多卡
	7. Android 修改音频gpio
	8. Android lan1 lan1 led 灯配置
...
```
信步：调试 1v4 USB 转串口。
通过网盘分享的文件：S624-10+V16-A11-EL-AB-QY-20260410.zip
链接: https://pan.baidu.com/s/1F5umaqsMAa0q-EXQSCEYsw?pwd=hdp4 提取码: hdp4
```



JXCekuhR6HzwsJI5BVv//xv3ft38M+kZHcnxhd5Izl4= 

http://192.168.31.224:9008/ root

L1klpfUHeyF37s24p3SjI46EGv2VGNre2RhdjzvYEJ8= 
...
232 芯片的适配。  xr21v1414
 ...
 export PATH=/usr/local/gstreamer/bin:$PATHroot@user:-# export LD_LIBRARY PATH=/us r/local/gstreamer/lib/aarch64-linux-gnu: $LD_LIBRARY PATHroot@user:-# export GST_PLUGIN PATH=/usr/local/gstreamer/lib/aarch64-linux-gnu/gstreamer-1.0
 
 
 export PATH=/usr/local/bin:$PATH
 export LD_LIBRARY_PATH=/usr/lib/aarch64-linux-gnu:$LD_LIBRARY_PATH
 export GST_PLUGIN_PATH=/usr/lib/aarch64-linux-gnu/gstreamer-1.0
 gst-inspect-1.0 I grep mpp
...
```
放电：一般20分钟后，就亮了红灯了。然后保持待机15-20分钟时间。
	1. 开始电压：12.3V  结束电压：9.2V  开始 8：26  结束：9：08 时长：40分钟  搁置一夜。接显示屏。
	1. 开始电压：12.5V  结束电压：8.9V  开始 9：11  结束：9: 52 时长：41分钟 
	1. 开始电压：12.3V  结束电压：9.6V  开始 9：57  结束：10：32 时长：分钟 35 分钟
	1. 开始电压：12.1V  结束电压：8.7V 开始 10：58  结束：11：39 时长：分钟 40 分钟
	1. 开始 13:05  结束：14:42 37分钟
	1. 开始 14:10  结束：14：45 35分钟
	1. 开始 15:05  结束：15：50 45 分钟
	1. 开始 15:53  结束：16：20 27 分钟
	1. 开始 16:43  结束：17：20 37 分钟
	1. 开始 17:28  结束：17：58 30 分钟
	
	2. 开始电压：12.3V  结束电压：8.6V  开始 8：28  结束：9：13 时长：45分钟  搁置一夜。没有接显示屏
	2. 开始电压：12.5V  结束电压：8.7V  开始 9：17  结束：10:03 时长：46分钟  没有接显示屏
	2. 开始电压：12.3V  结束电压：8.9V  开始 10:07  结束：10:55 时长：48分钟  没有接显示屏
	2. 开始电压：12.3V  结束电压：9.3V  开始 10：58  结束：11：42 时长：44分钟  没有接显示屏
	2. 开始 13:05  结束：13:52 47分钟 结束电压 9.0V
	2. 开始 14:10  结束：14：50
	2. 开始 15:05  结束：15：45 40分钟
	2. 开始 17:28  结束：17：58 30分钟
	
	3. 开始 13:05  结束：13:50 45分钟 结束电压 8.7V
	3. 开始 14:10  结束：14：50 40分钟
	3. 开始 15:05  结束：15：40 35分钟
	
	4. 开始 13:05  结束：14：02  57 分钟 结束电压 8.7V
    4. 开始 14:10  结束：15：00 50 分钟
    4. 开始 15:05  结束：15：55 50 分钟
	
	
充电：
	2. 开始 16:45  结束：
	
	3. 开始电压：8.6V  结束电压：12.3V  开始 9：21  结束：10:02 时长：40  搁置一夜。30 分钟 12.1V 40 分钟 12.3V
	3. 开始电压：8.6V  结束电压：12.3V  开始 10：11  结束：10：41 时长：40  4颗灯全亮 10：37 30分钟 12.1V 后期充电比较慢
	3. 开始 10：03  结束：10：41 时长：40 
	3. 开始 15：56 结束：
	3. 开始 17：16 结束：
	
	4. 开始电压：8.6V  结束电压：12.3V  开始 9：21  结束：10:02 时长：40  30 分钟 12.1V 40 分钟 12.3V
	4. 开始电压：8.6V  结束电压：12.4V 开始 10：11  结束：10：41 时长：40
    4. 开始 10：03  结束：10：41 时长：40
    4. 开始 15：56 结束：
    4. 开始 17：16 结束：
```
