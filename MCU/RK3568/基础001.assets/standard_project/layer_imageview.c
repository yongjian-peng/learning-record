...
char imageViewPath[PATH_MAX];
bool imageViewLoading, imageViewLoaded;
static uint8_t* imageViewData;
static int imageViewDataSize;
extern int imagePlayerPageIndex, imagePlayerFocusIndex;
extern char* viewIconFilePath;
...
static void imageViewLoadCallback(uint8_t* data, int size)
{
    if (data && size > 0)
    {
        ...
        {
			//printf("imageView free data\n");
            free(imageViewData);
            imageViewData = NULL;
            imageViewDataSize = 0;
        }
		//printf("imageView data[0] %x data[1] %x\n", data[0], data[1]);
        imageViewData = data;
...
bool ImageViewLastButtonOnPress(ITUWidget* widget, char* param)
{
    ...
    if (item)
    {
        ...
        strncpy(imageViewPath, filepath, PATH_MAX);
...
bool ImageViewOnTimer(ITUWidget* widget, char* param)
{
    ...
    if (imageViewLoading && !imageViewLoaded)
    {
        int ret = PhotoLoad(imageViewPath, imageViewLoadCallback);
        if (ret == 0)
            imageViewLoading = false;
    }
    else if (imageViewLoaded)
    {
		//printf("imageViewOnTimer data[0] %x data[1] %x\n", imageViewData[0], imageViewData[1]);
...
bool ImageViewOnEnter(ITUWidget* widget, char* param)
{
	...
	if (!imageViewLoading)
	{
		...
		printf("[%s] Try to load %s\n", __func__,viewIconFilePath);
		strncpy(imageViewPath, viewIconFilePath, PATH_MAX);
