...
#ifdef CFG_UPGRADE_DOWNLOAD_STORAGE
    static ITCBlockStream blockStream;
    #if defined(CFG_UPGRADE_DOWNLOAD_NAND)
        static const char* blockDevname = ":nand";
    #elif defined(CFG_UPGRADE_DOWNLOAD_NOR)
    static const char* blockDevname = ":nor";
    #elif defined(CFG_UPGRADE_DOWNLOAD_SD0)
    static const char* blockDevname = ":sd0";
    #elif defined(CFG_UPGRADE_DOWNLOAD_SD1)
    static const char* blockDevname = ":sd1";
    #endif
#else
    static uint8_t* fwFileBuf;
#endif // CFG_UPGRADE_DOWNLOAD_STORAGE
	       ...
	       const char *data, uint64_t off, size_t size)
{
    ...
    if (!request->filebuf)
    {
        ...
        if (itcBlockStreamOpen(&blockStream, blockDevname, CFG_UPGRADE_DOWNLOAD_POS, strtoul(clen, NULL, 10), true) == 0)
        {
            printf("open block stream %s to write\n", blockDevname);
            request->filebuf = (uint8_t*)&blockStream;
        }
        else
        {
            printf("try to open block device %s to write fail:0x%X\n", blockDevname, errno);
            return MHD_NO;
        }
    }

    off = itcStreamWrite(&blockStream, data, size);
    if (off != size)
    {
        printf("Cannot write file: %ld != %ld\n", off, size);
        return MHD_NO;
    }
        ...
        if (0 == strcmp (method, MHD_HTTP_METHOD_GET))
        {
            ...
            *ptr = NULL;                  /* reset when done */
            ...
            if (strcmp(val, "upgrade") == 0)
            {
                ...
                // upgrade firmware
            #ifdef CFG_UPGRADE_DOWNLOAD_STORAGE
                if (itcBlockStreamOpen(&blockStream, blockDevname, CFG_UPGRADE_DOWNLOAD_POS, fwFileSize, false) == 0)
                {
                    if (ugCheckCrc(&blockStream.stream, NULL))
                    {
                        puts("Upgrade failed.");
                        itcStreamClose(&blockStream);
                    }
                    else
                    {
                        UpgradeSetStream(&blockStream.stream);
                ...
                {
                    printf("try to open %s to read fail\n", blockDevname);
                }
            #else
