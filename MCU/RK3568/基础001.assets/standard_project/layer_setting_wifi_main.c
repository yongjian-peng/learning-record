...
static ITURadioBox* settingWiFiEthernetConnectRadioBox;
...
bool SettingWiFiMainRadioBoxOnPress(ITUWidget* widget, char* param)
{
	...
	else if (ituRadioBoxIsChecked(settingWiFiEthernetConnectRadioBox))
	{
		//theConfig.wifi_mode = WIFIMGR_MODE_MAX;
		//theConfig.wifi_on_off = WIFIMGR_SWITCH_ON;
//#ifdef CFG_NET_WIFI
//		WifiMgr_Sta_Switch(theConfig.wifi_on_off);
//#endif
		ituSpriteGoto(settingWiFiMainTypeSprite, 3);
	}
...
bool SettingWiFiMainOnEnter(ITUWidget* widget, char* param)
{
    if (!settingWiFiMainTypeSprite)
    {
		...
		settingWiFiEthernetConnectRadioBox = ituSceneFindWidget(&theScene, "settingWiFiEthernetConnectRadioBox");
		assert(settingWiFiEthernetConnectRadioBox);
    ...
    case 2:
        ...
        break;
	case 3:
		ituRadioBoxSetChecked(settingWiFiEthernetConnectRadioBox, true);
		ituSpriteGoto(settingWiFiMainTypeSprite, 3);
		break;
