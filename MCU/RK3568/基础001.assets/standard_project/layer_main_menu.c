...
#define THE_SEC_RIGHT_LIMITE 633
...
static ITUShadow *mainMenuSubShadow[2][MENU_TOTAL_APP];
static ITUContainer *mainMenuSubContainer[2][MENU_TOTAL_APP];

#if CFG_LCD_WIDTH == 1280
static ITUText *mainMenuSubText[2][MENU_TOTAL_APP];
#endif

static ITUContainer *mainMenuPFSubContainer[2][MENU_TOTAL_APP];
static ITUContainer *mainMenuTmpContainer[MENU_TOTAL_APP];
static ITUButton *mainMenuTmpButton[MENU_TOTAL_APP];
static ITUAnimation *mainMenuTmpAnimation[MENU_TOTAL_APP];
static ITUAnimation *mainMenuSubAnimation[2][MENU_TOTAL_APP];
static ITUPopupButton *mainMenuSubPopupButton[2][MENU_TOTAL_APP];
...
{		
	...
	if (longPressState == true) {
		...
		if (page_index == 1)
		{
			if (ituWidgetGetX(mainMenuTmpContainer[press_index]) > THE_SEC_RIGHT_LIMITE)
				curPos = 3;
		}
#endif
...
#if CFG_LCD_WIDTH == 1280
				ituWidgetSetVisible(mainMenuSubContainer[1][theConfig.order[1][3]], false);
				ituWidgetSetVisible(mainMenuPFSubContainer[1][theConfig.order[1][3]], false);
#else
...
bool MainMenuCoverFlowOnChange(ITUWidget* widget, char* param)
{
	...
	if (longPressState == true) {
		...
		if (next == 1)
		{
			tmp = theConfig.order[next][3];
			theConfig.order[next][3] = theConfig.order[prev][5];
			theConfig.order[prev][5] = tmp;
			ituWidgetSetVisible(mainMenuSubContainer[prev][theConfig.order[prev][5]], true);
			ituWidgetSetVisible(mainMenuPFSubContainer[prev][theConfig.order[prev][5]], true);
			lastPos = 3;
		...
		{
			//tmp = theConfig.order[next][5];
			//theConfig.order[next][5] = theConfig.order[prev][3];
			//theConfig.order[prev][3] = tmp;
			//ituWidgetSetVisible(mainMenuSubContainer[prev][theConfig.order[prev][3]], true);
			//ituWidgetSetVisible(mainMenuPFSubContainer[prev][theConfig.order[prev][3]], true);
			//lastPos = 5;

			tmp = theConfig.order[prev][0];
			theConfig.order[prev][0] = theConfig.order[prev][1];
			theConfig.order[prev][1] = tmp;

			tmp = theConfig.order[prev][1];
			theConfig.order[prev][1] = theConfig.order[prev][2];
			theConfig.order[prev][2] = tmp;

			tmp = theConfig.order[prev][2];
			theConfig.order[prev][2] = theConfig.order[prev][3];
			theConfig.order[prev][3] = tmp;
