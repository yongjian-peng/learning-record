...
function updateCtrlBoardSetting() {
    ...
    var $screensaver_type = $screensaver_type = document.getElementById("screensaver_type").selectedIndex + 1;
    ...
    if (confirm("Do you want to update setting?")==true) {
        ...
        var nocache = "&nocache=" + Math.random() * 1000000;
        var hostStr = "http://" + location.host + "/dev/info.cgi?action=setting&brightness=" + $brightness + "&screensaver_time=" + $screensaver_time + "&screensaver_type=" + $screensaver_type + "&lang=" + $lang + "&keylevel=" + $keylevel + "&play_lev=" + $play_lev + nocache;
