...
#ifdef CFG_ENABLE_ROTATE
        sprintf(filepath, CFG_PRIVATE_DRIVE ":/itu/%ux%u/standard_project.itu", ithLcdGetHeight(), ithLcdGetWidth());
#else
        sprintf(filepath, CFG_PRIVATE_DRIVE ":/itu/%ux%u/standard_project.itu", ithLcdGetWidth(), ithLcdGetHeight());
#endif
        ituSceneLoadFileCore(&theScene, filepath);
    }
    #else
    ituSceneLoadFileCore(&theScene, CFG_PRIVATE_DRIVE ":/standard_project.itu");
