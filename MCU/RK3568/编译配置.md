...
/home/yxw/myproject/rk356x_linux5.1/buildroot/output/rockchip_rk3568/build/qt5base-e44097b63d17ba3178a637df7fac51ddc51cb48b/bin/qmake
...
ls /dev/ds18b20 检查是否创建节点, 创建设备节点的目的是，给应用层操作硬件的函数

ls /sys/class/sensors/ds18b20 检查是否创建 类

cat /proc/devices/  检查设备号

cat /proc/bus/input/devices 对应芯片的信息

// 使用C程序，用 id_table 
cat /sys/bus/platform/devices/fe5a000.i2c/i2c-1/1-0038/name // 验证 i2c 是否使用设备检点 使用原生调用接口实现验证。


ls /dev/input/event2 对应设备节点

cat /proc/device-tree/i2c... // 查看设备节点是否加入内核中
...
命令	用途	特点
fdisk -l	查看MBR分区表	传统，适合小磁盘
parted -l	查看MBR/GPT分区表	支持GPT，功能更强大
lsblk	查看块设备树状图	直观，显示层次关系
blkid	查看文件系统UUID和类型	显示文件系统信息
df -h	查看已挂载的文件系统	显示使用情况
```
...
![image-20251103095906015](./编译配置.assets/image-20251103095906015.png)
...
KDIR :=/home/yxw/myproject/rk356x_linux5.1/kernel #内核源码所在虚拟机ubuntu的实际路径
PWD ?= $(shell pwd)
all:
	make -C $(KDIR) M=$(PWD) modules    #make操作
clean:
