...
```
设备模型
PWM 使用的是 平台总线 字符设备驱动
	寄存器控制 PWM 和 软件控制 PWM 模拟控制 PWM  通过控制 GPIO 高低电平的比例。
I2C 用的是 I2C 总线
SPI 用的是 SPI 总线
UART 串口
看门狗：是字符设备驱动。
...
obj-m += wq.o   
KDIR :=/home/yxw/myproject/rk356x_linux5.1/kernel
PWD ?= $(shell pwd)
all:
...
/home/yxw/myproject/rk356x_linux5.1/prebuilts/gcc/linux-x86/aarch64/gcc-arm-10.3-2021.07-x86_64-aarch64-none-linux-gnu/bin/aarch64-none-linux-gnu-gcc app.c -o app
	
	
sudo apt install gcc-12 g++-12
# KBUILD_CFLAGS_MODULE += -DDEBUG
export ARCH=x86_64#arm64#设置平台架构
#export CROSS_COMPILE=home/yxw/myproject/rk356x_linux5.1/prebuilts/gcc/linux-x86/aarch64/gcc-arm-10.3-2021.07-x86_64-aarch64-none-linux-gnu/bin/aarch64-none-linux-gnu- #交叉编译器前缀
obj-m += define.o	#helloworld.c对应.o文件的名称。名称要保持一致。
KDIR :=/lib/modules/6.8.0-84-generic/build    #内核源码所在虚拟机ubuntu的实际路径
PWD ?= $(shell pwd)
all:
...
// 编译到 ubuntu 系统中 KDIR:=/lib/modules/5.4.0-110-generic/build 
```
...
![image-20250901195848975](./基础001.assets/image-20250901195848975.png)



![image-20250901195900087](./基础001.assets/image-20250901195900087.png)

![image-20250901195911214](./基础001.assets/image-20250901195911214.png)
...
![image-20251213160446153](./基础001.assets/image-20251213160446153.png)
...
![image-20250924162103239](./基础001.assets/image-20250924162103239.png)
...
```
输入设备在 /dev/input/event4  查看输入设备 cat /sys/class/input/event4/device/name  
GPIO 子系统知识  中断知识，和输入子系统知识
// 中断触发条件
ret = request_irq(client->irq, ft5x06_handler,
                     IRQ_TYPE_EDGE_FALLING | IRQF_ONESHOT, "ft5x06 irq", NULL);
// 监测是否有数据上报 
hexdump /dev/input/event4  
打印数据信息分析 input-event-codes.h
// 编写驱动中断号，赋值中断号 。 prob函数。 匹配成功后，会先执行总线中的prob 函数
```
...
![image-20250911205322617](./基础001.assets/image-20250911205322617.png)
...
![image-20260410203947357](E:\Git\learning-record\MCU\RK3568\assets\image-20260410203947357.png)

![image-20260410204408266](E:\Git\learning-record\MCU\RK3568\assets\image-20260410204408266.png)
