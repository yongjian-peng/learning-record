...
/ {
	...
	gpio-keys {
	    ...
	    #address-cells = <1>;      // 新增：地址单元数（用于子节点寻址）
		#size-cells = <0>;         // 新增：大小单元数（配合地址单元）
		input-name = "gpio-keys";  // 新增：明确输入设备名称（内核识别用）
		status = "okay";           // 新增：启用该节点（部分设备树默认禁用）
        ...
        otg-switch-key {
		    ...
		    debounce-interval = <50>;  // 去抖动（必须，否则按键乱触发）
        	linux,input-type = <1>;    // 新增：指定为按键类型（1=EV_KEY）
    		wakeup = <0>;              // 可选：0=不支持唤醒系统（简化配置）
        };
    };
